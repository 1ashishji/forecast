class SalaryInsightsService
  def self.country_stats(country)
    Rails.cache.fetch("salary_stats/#{country}", expires_in: 1.hour) do
  ...
  def self.job_title_stats(country, job_title)
    Rails.cache.fetch("salary_stats/#{country}/#{job_title}", expires_in: 1.hour) do
  ...
  def self.advanced_metrics(country)
    Rails.cache.fetch("advanced_metrics/#{country}", expires_in: 1.hour) do
  ...
  def self.clear_cache(country)
    Rails.cache.delete_matched("salary_stats/#{country}*")
    Rails.cache.delete("advanced_metrics/#{country}")
  end
end
